...
is assumed to be already sorted in increasing order as it is received.
See the flowchart to understand more easily how this function works. 
'''
...
if os.path.exists(sourceFolder):
    ...
    print('\nWill insert gaps in numbering to text files'\
          ' starting with "' + startStr + '" if necessary:')
    print(sourceFolder, '\n')
else:
...
serList = orSerList.copy() # this is the list of files to be manipulated
serList.sort() # sort the list to get ready for the insert_gap function


# Initiate mapping of series with existing serial numbers.
# This was done to make the code easier to follow and to be able to use
...
# insList is not updated at all during the modification of serial
# numbers because once a gap is opened, the next intersection set
# automatically no longer includes the insertion for which the gap was
# created.
# The updating of the intersection set every time an insertion necessity
# is tested and starting of modification from the smallest insertion in
# the set guarantees gaps for all insertions.
